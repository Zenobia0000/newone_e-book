...
> **評鑑範圍**：初級 L121（No Code/Low Code 概念）+ L122（GenAI 應用領域與工具），合計佔科目二約 30-40%
...
```
00:00-00:08  開場暖身：GenAI 工具考試佔比 + 核心問題引入
00:08-00:20  No Code / Low Code 概念與差異
00:20-00:40  GenAI 工具全景：ChatGPT/API/Copilot 系列/Midjourney/Gemini
00:40-00:58  提示工程：框架/技巧/判斷好壞
00:58-01:15  RAG 原理 + RAG vs 微調選型
01:15-01:25  AI 工具整合 + 三層判斷法收束
01:25-01:30  銜接 Ch05
...
1. **No Code vs Low Code 是必考送分題**：只要記住一個差異——No Code 完全不寫程式，Low Code 需要少量程式。考題會用各種方式包裝，但核心就是這一點。

2. **GenAI 工具要建地圖不要背清單**：工具太多背不完。用四象限分類（文字/圖片/程式/流程）建立心智地圖，考題給你場景時直接定位到象限再選工具。

3. **Copilot 家族容易搞混**：GitHub Copilot 是寫程式的，Copilot Studio 是做流程自動化的，Microsoft 365 Copilot 是辦公軟體助手。名字都有 Copilot 但定位完全不同，這是高頻考點。

4. **提示工程用 RTFC 框架教**：Role-Task-Format-Constraint 四要素。不用背複雜方法論，記住這四個就能判斷提示好壞。考題會給兩段提示問哪個比較好——有 RTFC 結構的就是好提示。
...
A：考試考的是概念和判斷力，不是特定版本。理解工具的「類型」和「定位」比記住特定功能重要。
