...
- **統計學的缺失值分類（MCAR / MAR / MNAR）**：
  - **MCAR（Missing Completely At Random）**：缺失和任何變數都無關。例如問卷被風吹走。可以直接刪除，不會產生偏差。
  - **MAR（Missing At Random）**：缺失和「其他已知變數」有關，但和缺失值本身無關。例如年輕人較不填收入欄位。可以用其他變數來推估填補。
  - **MNAR（Missing Not At Random）**：缺失和「缺失值本身」有關。例如高收入者刻意不填收入。最棘手，填補可能產生偏差。
  - **初學者實用判斷**：大部分情況你不需要精確分類，但要問自己：「這些缺失是隨機的，還是有系統性原因？」如果有系統性原因，簡單刪除或填補都可能讓分析結果偏掉。
...
### pandas 官方資源
1. **pandas 2.0 Release Notes (April 3, 2023)** — Arrow backend、Copy-on-Write 語意變更。[pandas.pydata.org/docs/whatsnew/v2.0.0.html](https://pandas.pydata.org/docs/whatsnew/v2.0.0.html)
2. **pandas Official Documentation** — DataFrame API 完整說明。[pandas.pydata.org/docs](https://pandas.pydata.org/docs/)
3. **Arrow backend 記憶體改善** — PyArrow 字串欄位相比 NumPy object 欄位可減少最多 70% 記憶體；整體使用 Arrow dtype_backend 約減少 50% 記憶體。來源：pandas 官方文件與社群基準測試。參見 [datapythonista.me/blog/pandas-20-and-the-arrow-revolution-part-i](https://datapythonista.me/blog/pandas-20-and-the-arrow-revolution-part-i)

### 缺失值理論
4. **MCAR / MAR / MNAR 分類** — 原始定義來自 Rubin, D.B. (1976). "Inference and Missing Data." *Biometrika*, 63(3), 581-592. 此三分類是缺失值處理的統計學標準框架。
5. **Little, R.J.A. & Rubin, D.B. (2019). *Statistical Analysis with Missing Data*, 3rd Edition. Wiley.** — 缺失值分析的權威教科書。

### 缺失值處理決策門檻
6. **缺失比例 < 5% / 5-30% / > 30% 門檻** — 此為資料科學實務中常用的經驗法則，非絕對標準。參考來源：Schafer, J.L. (1999). "Multiple imputation: a primer." *Statistical Methods in Medical Research*, 8(1), 3-15. 以及 Bennett, D.A. (2001). "How can I deal with missing data in my study?" *Australian and New Zealand Journal of Public Health*, 25(5), 464-469.

### 工具生態
7. **Polars Documentation** — 高效能 DataFrame 替代方案。[docs.pola.rs](https://docs.pola.rs/)
8. **DuckDB Documentation** — 嵌入式分析資料庫。[duckdb.org/docs](https://duckdb.org/docs/)
9. **Apache Arrow** — 跨語言欄式記憶體格式。[arrow.apache.org](https://arrow.apache.org/)

### 延伸閱讀
10. McKinney, W. (2022). *Python for Data Analysis*, 3rd Edition. O'Reilly Media. — pandas 作者親撰的權威指南。
11. Wickham, H. (2014). "Tidy Data." *Journal of Statistical Software*, 59(10). — 整潔資料原則。[doi.org/10.18637/jss.v059.i10](https://doi.org/10.18637/jss.v059.i10)
