...
### 經典文獻
1. **Tukey, J.W. (1977). *Exploratory Data Analysis*. Pearson.** — EDA 方法論的奠基之作。「The greatest value of a picture is when it forces us to notice what we never expected to see.」出自本書第 vi 頁。
2. **Anscombe, F.J. (1973). "Graphs in Statistical Analysis." *The American Statistician*, 27(1), 17-21.** — Anscombe's Quartet 的原始論文，示範統計量相同但分布截然不同的四組資料。[doi.org/10.1080/00031305.1973.10478966](https://doi.org/10.1080/00031305.1973.10478966)

### 視覺化原則
3. **Tufte, E.R. (2001). *The Visual Display of Quantitative Information*, 2nd Edition. Graphics Press.** — 資料視覺化設計的聖經，「data-ink ratio」原則的來源。
4. **Cairo, A. (2016). *The Truthful Art: Data, Charts, and Maps for Communication*. New Riders.** — 資料新聞與商業視覺化的實務指南。
5. **Knaflic, C.N. (2015). *Storytelling with Data*. Wiley.** — 「用資料說故事」的商業簡報方法論。

### 洞察框架
6. **What -> So What -> Now What 框架** — 此框架廣泛使用於管理顧問業（McKinsey、BCG 等）與企業資料團隊。最早的系統化描述可追溯至 Minto, B. (1987). *The Pyramid Principle*. Pearson. 該書提出的金字塔原理強調「結論先行、以上統下」的思維方式。

### 視覺化反模式
7. **圓餅圖的判讀問題** — Cleveland, W.S. & McGill, R. (1984). "Graphical Perception: Theory, Experimentation, and Application to the Development of Graphical Methods." *Journal of the American Statistical Association*, 79(387), 531-554. 此研究證實人眼對長度的判讀精度遠高於角度。
8. **色彩選擇** — viridis 色票的設計考量可參見 [matplotlib.org/stable/users/explain/colors/colormaps.html](https://matplotlib.org/stable/users/explain/colors/colormaps.html)

### 工具文件
9. **Matplotlib Official Documentation** — [matplotlib.org](https://matplotlib.org/)
10. **Seaborn Official Documentation** — [seaborn.pydata.org](https://seaborn.pydata.org/)
