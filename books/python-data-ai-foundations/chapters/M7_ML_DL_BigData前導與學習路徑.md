# Module 7：ML / DL / Big Data 前導與學習路徑

**課程總時長：** 3 小時（本課程最終模組）
**模組定位：** 在 6 個模組建立的資料分析底盤之上，打開前方三條技術縱深的視野，並以五條職涯路線為學員指出離開課程後的下一步。
...
| # | 標題 | 核心訊息 | 時段歸屬 |
|---|------|----------|----------|
| S01 | 你已經走了多遠 | 回顧六個模組建立的底盤 | Part A 開場 |
...
- 前六個模組不是在「學 Python」，而是在建立一套資料工程的共同語言：型別、陣列、表格、視覺化、統計直覺、系統底層
...
**視覺建議：** 一張學習路線圖，六個模組節點已點亮，第七個模組節點（本模組）位於圖的末端，後方分叉出五條路線箭頭，以漸層方式呈現「已完成」與「前方可走」的對比。
...
- 對初學者的意義：現在學 PyTorch，不只是學 `nn.Linear` / `nn.ReLU`，而是要有「這段代碼未來要跑在什麼環境」的系統工程直覺
...
**過渡：** 在這個梯度的末端，PySpark 是 Python 工程師進入大數據世界的主要橋梁。
...
- PySpark 不只是「大一點的 pandas」：它背後連接著整個 Hadoop 生態（HDFS、Hive、YARN）和雲端資料倉儲（BigQuery、Databricks、AWS EMR），進入這裡等於進入企業級資料工程的核心
...
**過渡：** 技術前導結束。現在你站在六條路的交叉口——讓我們把每條路說清楚。
...
- **雙主線回顧**：這門課從第一天就是兩條線交織進行的——資料與 AI 能力線，以及軟體與系統底層線。這不是偶然的設計，而是因為真實的 AI 工程師需要同時具備這兩種直覺
- **里程碑式進步**：從「能讀 CSV」到「能清理資料」到「能做統計分析」到「能建一個分類模型」到「知道當資料規模大了要換什麼工具」——這是一條可以量測的進步曲線
- **心法一**：「先會寫，再會組；先懂資料，再懂系統；先搭底盤，再碰 AI。」——這個順序不是保守，而是讓你後面走得更快的投資
- **心法二**：「You didn't learn a tool. You built a foundation.」——工具會換，但資料思維、系統直覺、程式設計能力不會換。你學的是那個不換的部分
...
**本課程已建立的前置底盤：** pandas 資料清理、統計基礎（Module 4-5）、視覺化呈現
...
**本課程已建立的前置底盤：** scikit-learn 工作流（本模組 Part A）、pandas 特徵處理、統計評估直覺
...
| 效能工程 | `torch.compile()`、混合精度訓練、分散式訓練基礎 |

**典型職位：** 深度學習工程師、電腦視覺工程師、NLP 工程師、AI 研究員

**本課程已建立的前置底盤：** NumPy tensor 概念、PyTorch 前導（本模組 Part B）、OOP 設計直覺
...
**本課程已建立的前置底盤：** pandas 資料處理、PySpark 前導（本模組 Part B）、Python 系統底層直覺（模組 5-6）
...
**本課程已建立的前置底盤：** Python OOP（用於 Agent 類別設計）、pandas 資料處理（文件預處理）、Python I/O 與系統直覺（API 整合）
...
這 24 小時的課程從第一天就是兩條線交織進行的：

```
主線 A：資料與 AI 能力線          主線 B：軟體與系統底層線
─────────────────────          ─────────────────────
Python 基礎與型別系統            計算思維與演算法直覺
NumPy 數值計算                   OOP 與模組化設計
pandas 表格處理與 EDA            虛擬環境與相依管理
統計思維與視覺化                  例外處理與 logging
ML 前導：建模工作流               Process / Thread / Memory
DL 前導：tensor 到神經網路        CPU / GPU 執行模型
Big Data 工具選型                 Linux 與 OS 底層直覺
```

這兩條線的交會點，就是真實 AI 工程的工作現場：

- pandas 大表格 → 連到記憶體管理的直覺
- PyTorch tensor → 連到 GPU / autograd 的理解
- DataLoader → 連到 process / I/O 的架構直覺
- PySpark → 連到分散式執行模型的系統概念
...
工具會換，框架會更新，API 會改版，但資料思維、系統直覺、程式設計能力不會換。你在這 24 小時裡學的，是那個不換的部分。
